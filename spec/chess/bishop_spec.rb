...
describe Chess::Bishop do
  describe '#generate_adjacent_movement_coords' do
    ...
    context 'when passed AlgebraicCoords e4' do
      ...
      it 'returns a hash of adjacent coordinates per direction' do
        result = bishop.generate_adjacent_movement_coords(algebraic_coords_e4)
    ...
    context 'when passed AlgebraicCoords a8' do
      ...
      it 'returns a hash of adjacent coordinates per direction' do
        result = bishop.generate_adjacent_movement_coords(algebraic_coords_a8)
        expect(result).to eq(expected)
      end
    end
  end
  ...
  describe '#generate_adjacent_capture_coords' do
    ...
    it 'sends the #generate_adjacent_movement_coords message to self with the passed AlgebraicCoords' do
      allow(bishop).to receive(:generate_adjacent_movement_coords).with(algebraic_coords)
      bishop.generate_adjacent_capture_coords(algebraic_coords)
      expect(bishop).to have_received(:generate_adjacent_movement_coords).with(algebraic_coords)
    end
    # rubocop: enable all
  end
end
