...
class PositionSerializer
  ...
  def serialize_rank(rank) # rubocop:disable Metrics/MethodLength
    Position.rank_keys(rank).each_with_object([]) do |algebraic_coords, arr|
      square = @position.state[algebraic_coords]
      if square.empty?
        increment_contiguous_empty_square_counter
        arr << @contiguous_empty_square_counter.to_s if h_file?(algebraic_coords)
  ...
  def h_file?(algebraic_coords)
    algebraic_coords.to_s[0] == Constants::BOARD_FILES.last
  end
